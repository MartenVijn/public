...
def on_message(mqc, userdata , msg ):
  payload=msg.payload
  if ( msg.topic == "sarah/knop" ):
    now = datetime.now()
    begin = now.replace(hour=7, minute=0, second=0, microsecond=0)
    eind = now.replace(hour=20, minute=0, second=0, microsecond=0)
    if begin < now < eind:
      mqc.publish("sarah/kidled",payload)
#      mqc.publish("sarah/led","on")
#   else:
#     mqc.publish("sarah/led/","off")
    mqc.publish("sarah/parentled",payload)
